...
impl BlockMetadata {
    ...
    pub fn encode(&self) -> Vec<u8> {
        ...
        // size of first key
        let first_key_size: u16 = self.first_key.len().try_into().expect("size must fit in 2 bytes");
        encoded.extend(first_key_size.to_be_bytes());
        encoded.extend(&self.first_key);
        // size of last key
        let last_key_size: u16 = self.last_key.len().try_into().expect("size must fit in 2 bytes");
    ...
    pub fn decode(encoded_block_meta: Vec<u8>) -> Self {
        ...
        current_index += 2;
        let first_key = Bytes::copy_from_slice(&encoded_block_meta[current_index..current_index+first_key_size]);
        current_index += first_key_size;
        let last_key_size: usize = u16::from_be_bytes(encoded_block_meta[current_index..current_index+2].try_into().expect("chunk of size 2")).into();
        current_index += 2;
        let last_key = Bytes::copy_from_slice(&encoded_block_meta[current_index..current_index+last_key_size]);
...
mod tests {
    ...
    fn test_encode_decode() {
        ...
        let decoded_block_meta = BlockMetadata::decode(actual);
